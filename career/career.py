...
def compute(path):
    ...
    # primive greedy algo
    distances, path_predecessors = Graph.bellman_ford(graph, start)
...
class Graph:
        ...
        with open(path, "r") as input_file:
            ...
            for level in xrange(levels - 1):
                ...
                for index in xrange(len(pointer)):
                    ...
                    den1.inbound_edges.append(one)
                    den2.inbound_edges.append(two)
                    edges.append(one)
                    edges.append(two)
    ...
    @classmethod
    def bellman_ford(cls, graph, start_vertex):
        ...
        distances = OrderedDict((vertex.label, INFINITY) for vertex in graph.vertices)
        distances[start_vertex.label] = 0
        predecessors = OrderedDict((vertex.label, None) for vertex in graph.vertices)
        ...
        for edge_budget in range(1, len(graph.vertices) + 1):
            ...
            for vertex in graph.vertices:
                ...
                if case2_distance > case1_distance:
                    ...
                    predecessors[vertex.label] = better_predecessor

                distances[vertex.label] = max(case1_distance, case2_distance)
